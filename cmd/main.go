...
func main() {
	...
	// this setup is not recommended for production. To use cert-manager-managed certs, enable the
	// [METRICS-WITH-CERTS] block in config/default/kustomization.yaml and the
	// [PROMETHEUS-WITH-CERTS] block in config/prometheus/kustomization.yaml.
	...
	if err := (&controller.SonarQubeProjectReconciler{
		...
		setupLog.Error(err, "Failed to create controller", "controller", "SonarQubeProject")
		os.Exit(1)
	}
	...
	if err := (&controller.SonarQubeGroupReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("sonarqubegroup-controller"), //nolint:staticcheck
		NewSonarClient: func(baseURL, token string) sonarqube.Client {
			return sonarqube.NewClient(baseURL, token)
		},
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&controller.SonarQubeWebhookReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor("sonarqubewebhook-controller"), //nolint:staticcheck
		NewSonarClient: func(baseURL, token string) sonarqube.Client {
			return sonarqube.NewClient(baseURL, token)
		},
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "SonarQubeWebhook")
		os.Exit(1)
	}
	if enableWebhook {
